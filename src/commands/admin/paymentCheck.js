...
module.exports = {
  ...
  async execute(interaction, client) {
    if (!isOwnerOrStaff(interaction.member)) {
      await interaction.reply({ content: "Hanya staff yang bisa cek payment.", flags: MessageFlags.Ephemeral });
      return;
    }
    ...
    await interaction.reply({ content: payment ? `Payment \`${payment.id}\` diupdate ke \`${payment.status}\`.` : "Payment tidak ditemukan.", flags: MessageFlags.Ephemeral });
  },
};
