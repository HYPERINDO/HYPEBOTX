...
const { PAYMENT_STATUS, isPaymentPaid, isPaymentTerminal, normalizePaymentStatus } = require("../utils/paymentStatus");
...
function createPaymentService({
  ...
  async function handlePaymentProofMessage(message) {
    ...
    const alreadyPaid = ticketPayments.find((p) => isPaymentPaid(p.status));
    if (alreadyPaid) {
  ...
  async function sweepUnpaidOrdersForReminder({ client, thresholdMs, cooldownMs, maxAlerts } = {}) {
    ...
    const targets = allPayments
      .filter((p) => {
        ...
        return Boolean(status) && !isPaymentTerminal(status);
      })
      .filter((p) => {
  ...
  async function approvePaymentFromTicketId(interaction, relatedTicket, ticketIdSuffix = "") {
    ...
    const nextPaymentStatus = PAYMENT_STATUS.PAID;
  ...
  async function rejectPaymentFromTicketId(interaction, relatedTicket, ticketIdSuffix = "", reason = "") {
    ...
    const nextPaymentStatus = PAYMENT_STATUS.CANCELLED;
    ...
    if (statusSyncService?.syncTicketOrderQueueStatus) {
      ...
      await repositories.ticketRepository.update(resolvedTicketId, { orderStatus: PAYMENT_STATUS.CANCELLED }).catch(() => null);
    }
  ...
  async function setPaymentStatus({ guild, actorUser, paymentId, status = "paid", note = "" }) {
    ...
    if (!payment.ticketId) {
      ...
      const nextPaymentStatus = normalized === PAYMENT_STATUS.PAID ? PAYMENT_STATUS.PAID : normalized === PAYMENT_STATUS.CANCELLED ? PAYMENT_STATUS.CANCELLED : null;
    ...
    if (normalized === PAYMENT_STATUS.CANCELLED) {
  ...
  function normalizeStatus(raw) {
    const normalized = normalizePaymentStatus(raw, String(raw ?? "").toLowerCase().trim());
    if (normalized === PAYMENT_STATUS.FAILED) return PAYMENT_STATUS.CANCELLED;
    return normalized;
  }
